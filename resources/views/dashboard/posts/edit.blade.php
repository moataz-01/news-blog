...
@section('body')
    ...
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active">{{ __('words.editPost') }}</li>
        ...
        <li class="breadcrumb-menu">
            <div class="btn-group" role="group" aria-label="Button group with nested dropdown">
                ...
                <a class="btn btn-secondary" href="#"><i class="icon-settings"></i> &nbsp;{{ __('words.editPost') }}</a>
            </div>
        </li>
    </ol>
                                    ...
                                    @foreach (config('app.languages') as $key => $lang)
                                        <div class="tab-pane mt-3 fade @if ($loop->index == 0) show active in @endif"
                                            ...
                                            <div class="form-group col-md-12">
                                                ...
                                                <textarea name="{{$key}}[content]" class="form-control" cols="30" rows="10" id="editor">{{$post->translate($key)->content}}</textarea>
                                            </div>
                                            <div class="form-group col-md-12">
                                                <label>{{ __('words.smallDesc') }}</label>
                                                <textarea name="{{$key}}[smallDesc]" class="form-control" cols="30" rows="10" id="editor">{{$post->translate($key)->smallDesc}}</textarea>
                                            </div>
                                            <div class="form-group col-md-12">
                                                <label>{{ __('words.tags') }}</label>
                                                <textarea name="{{$key}}[tags]" class="form-control" cols="30" rows="1">{{$post->translate($key)->tags}}</textarea>
                                            </div>
